package com.sulake.core.communication.messages
{
   ...
   public class MessageClassManager implements IMessageClassManager
   {
      ...
      public function getMessageComposerID(param1:IMessageComposer) : int
      {
         ...
         var _loc4_:* = null;
         var _loc2_:int = -1;
         for(_loc3_ in this.var_890)
         {
            _loc4_ = this.var_890[_loc3_] as Class;
            if(param1 is _loc4_)
            {
      ...
      public function toString() : String
      {
         ...
         for(_loc2_ in this.var_890)
         {
            _loc1_ += _loc2_ + " -> " + this.var_890[_loc2_] + "\n";
         }
         _loc1_ += "Registered Message Event Classes: \n";
         for(_loc2_ in this.var_719)
         {
            _loc1_ += _loc2_ + " -> " + this.var_719[_loc2_] + "\n";
         }
         return _loc1_;
      }
      ...
      private function registerMessageEvent(param1:int, param2:Class) : Boolean
      {
         ...
         if(_loc3_..implementsInterface.@type == this._messageEventInterface)
         {
            ...
            {
               this.var_719[param1] = [param2];
            }
            else
            {
               _loc6_ = this.var_719[param1];
               _loc6_.push(param2);
            }
            return true;
         }

package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1792:int = 3;
      
      public const const_1683:int = 2;
      ...
      private var var_2221:MarketplaceItemStats;
      ...
      {
         if(!this.var_158 || !this._catalog || !this._catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = this.var_158.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         this.showConfirmation(this.const_1720,_loc2_);
      }
      ...
      {
         if(param1.furniCategoryId != this.var_1844 || param1.furniTypeId != this.var_1848)
         {
            return;
         }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = this.var_158.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = this.var_158.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               this.var_158.add(parser.offerId,updateItem);
            }
            this.var_158.remove(parser.requestedOfferId);
            this.showConfirmation(this.const_1683,updateItem);
            this._visualization.listUpdatedNotify();
         }
      ...
      public function redeemSoldOffers() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            ...
            if(_loc3_.status == MarketPlaceOfferState.const_415)
            {
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         this.var_2031 = _loc3_.creditsWaiting;
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         if(this.var_158)
         {
            for each(_loc5_ in this.var_158)
            {
               if(_loc5_ != null)
               {
                  _loc5_.dispose();
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            this.var_158.add(_loc4_.offerId,_loc6_);
         }
         if(this._visualization == null)
         {
            return;
      ...
      {
         this.requestOffers(this.var_2224,this.var_2223,this.var_2220,this.var_2222);
      }
      ...
      private function isPosterItem(param1:MarketPlaceOfferData) : Boolean
      {
         ...
         if(param1.furniType == 2 && param1.stuffData != null)
         {
            ...
            {
               _loc4_ = _loc3_.name;
               if(_loc4_ != null && _loc4_ == const_1158)
               {
                  _loc2_ = true;
               }
            }
